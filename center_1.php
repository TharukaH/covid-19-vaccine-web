...
if(isset($_GET['edit']))
{
  ...
  $dbh=new PDO('mysql:host=localhost;dbname=citysmart','root','');
  $dbh->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
  $q="UPDATE center SET total=total-1 WHERE id=?";
  $sth=$dbh->prepare($q);
  $sth->execute([$id]);
  $dbh=null;
  $iid=$_GET['edit'];
  $dbh=new PDO('mysql:host=localhost;dbname=citysmart','root','');
  $dbh->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
  $q="DELETE FROM appoinment WHERE app_no=? AND center=?";
  $sth=$dbh->prepare($q);
  $sth->execute([$iid,$id]);
  $dbh=null;
}
      ...
      $table.='<td>'.$record['dose'].'</td>';
       $table.='<td>'.$record['telephone'].'</td>';
        $table.='<td>'.$record['datee'].'</td>';
         ...
         $table.='<td>'.'<a href="center_1.php?edit='.$record['app_no'].'" type="button" style="text-align:center;background-color:yellow;padding:7px 15px;text-decoration:none;display:inline-block" >Confirm</a>'.'</td>';
           $table.='</tr>';
  }
           echo $table;
